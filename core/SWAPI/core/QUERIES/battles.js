module.exports = async ( message ) => {
    ...
    try {
        ...
        SWAPI.report.dev("SWAPI: Querying battles")
        ...
        const cursor = db.collection('battles')
        ...
        while( await cursor.hasNext() ) {
            const freshBattles = await cursor.next()
            found = true
        ...
        if( !found && SWAPI.apiq >= 10 ) {
            ...
            const cache = db.collection('battles')
                .find({})
                .sort({updated:-1})
                .limit(1)
            
            while( await cache.hasNext() ) {
                const cacheBattles = await cursor.next()
                found = true
                process.send(JSON.stringify({
                    id:message.id,
                    data:{
                        result:cacheBattles,
                        error:null,
                        warning:"110 fetch/battles \"Result served from cache\""
                    }
                }))
        ...
        if( !found ) {
            ...
            SWAPI.report.dev("SWAPI: Fetching battles")
            
            let response = null
                
            try { 
                SWAPI.apiq++
                response = await SWAPI.api.client.fetchBattles({
            ...
            if( response.result ) {
                SWAPI.mongo.put('battles', {}, response.result)
                process.send(JSON.stringify({
                    id:message.id,
                    data:{
